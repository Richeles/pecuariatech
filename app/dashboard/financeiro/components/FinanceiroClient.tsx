...
export default function FinanceiroClient() {
  ...
  const tendenciaCapital = capitalScore > 80 ? "↑ +3%" : capitalScore > 60 ? "↑ +1%" : "→ Estável";
  ...
  const exportarRelatorio = async () => {
    try {
      ...
      a.href = url;
      a.download = `relatorio_financeiro_completo_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh] bg-[#0F2A1A]">
        <div className="animate-pulse text-[#A7F3D0]/60">Carregando dados financeiros...</div>
      </div>
    );
  }
    ...
    <section className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-[40px] border border-emerald-500/20 bg-gradient-to-br from-[#03140d] via-[#072117] to-[#0b2d1f] p-10 shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
        ...
        <div className="relative z-10">
          ...
          <h1 className="mt-6 text-5xl font-black tracking-tight text-white">Financeiro Inteligente</h1>
          <p className="mt-3 max-w-3xl text-lg leading-relaxed text-emerald-100/80">
            Runtime financeiro cognitivo com dados em tempo real
          </p>
        </div>
      </div>

      {/* IMPORTAÇÃO DE DADOS FINANCEIROS */}
      <UploadPlanilha tipo="financeiro" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm transition-all duration-200 hover:scale-[1.02]">
          ...
          <div className="text-xs text-emerald-500 mt-1">{tendenciaCapital}</div>
        </div>
