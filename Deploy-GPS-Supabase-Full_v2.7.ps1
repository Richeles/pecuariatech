<# 
 Deploy-GPS-Supabase-Full_v2.7.ps1
 --------------------------------------------------------
 ✔ Teste de conexão inteligente via HTTPS
 ✔ TLS 1.2/1.3 forçado internamente
 ✔ Alerta verde de sucesso e vermelho em erro real
...
# Garante que o PowerShell use TLS moderno (Supabase exige >= TLS 1.2)
...
function Test-SupabaseReachable {
    try {
        ...
        if ($_.Exception.Response -and ($_.Exception.Response.StatusCode.value__ -in 200,401,403)) {
            return $true  # Supabase respondeu, então está acessível
        }
        return $false
    }
}
...
function Run-MainLoop {
    Write-Host "== Deploy-GPS-Supabase-Full v2.7 ==" -ForegroundColor Cyan
    Ensure-Directories
    Init-CsvLog
