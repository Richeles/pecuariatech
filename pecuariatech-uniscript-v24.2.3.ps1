# ============================================================
# PECUARIATECH UNISCRIPT v24.2.3 - SCRIPT ÚNICO FINAL
...
function Enviar-WhatsApp {
    ...
    if (-not $WhatsNumber -or -not $WhatsToken) {
        Escrever-Log "WhatsApp não configurado, pulando." "WHATSAPP"
        return
    }
    ...
    try {
        ...
        Escrever-Log "WhatsApp enviado." "WHATSAPP"
    } catch {
...
function Testar-Ambiente {
    Escrever-Log "Iniciando diagnóstico do ambiente..." "CHECK"
    $ok = $true
...
function Testar-Permissoes {
    ...
    try {
        "X" | Out-File "$ROOT\test.tmp"
        Remove-Item "$ROOT\test.tmp" -Force
    } catch {
        Escrever-Log "Sem permissão na pasta ROOT." "ERROR"
        $ok = $false
    }

    try {
        "X" | Out-File $LOG -Append
    } catch {
        Escrever-Log "Sem permissão no LOG." "ERROR"
        $ok = $false
    }
...
# ====================================
# TRIÂNGULO 360 (DNS + HTTP + Ping)
# ====================================
function Testar-Rede {
    Escrever-Log "Testando rede..." "CHECK"
    $ok = $true

    $host = ([System.Uri]$SiteUrl).Host

    try { Resolve-DnsName $host | Out-Null; Escrever-Log "DNS OK ($host)." "CHECK" }
    catch { Escrever-Log "Falha DNS." "ERROR"; $ok=$false }

    try { Test-Connection $host -Count 1 -Quiet | Out-Null; Escrever-Log "Ping OK." "CHECK" } 
    catch { Escrever-Log "Ping falhou." "WARN" }

    try { Invoke-WebRequest -Uri $SiteUrl -TimeoutSec 10 | Out-Null; Escrever-Log "HTTP OK." "CHECK" }
    catch { Escrever-Log "HTTP falhou." "WARN"; $ok=$false }
...
function Enviar-Supabase {
    ...
    try {
        ...
        Escrever-Log "Supabase OK (status $Status)." "SUPABASE"
    } catch {
...
function Fazer-Deploy {
    Escrever-Log "Solicitação de deploy." "DEPLOY"

    $agora = Get-Date
    if (($agora - $global:LastDeploy).TotalMinutes -lt $MinutosEntreDeploys) {
        Escrever-Log "Ignorado (proteção anti-loop)." "WARN"
        return
    }

    $global:LastDeploy = $agora

    if (-not (Testar-Ambiente)) { Enviar-WhatsApp "Erro no ambiente."; return }
    if (-not (Testar-Permissoes)) { Enviar-WhatsApp "Permissões insuficientes."; return }

    Escrever-Log "Iniciando deploy..." "DEPLOY"
    Enviar-WhatsApp "Iniciando deploy automático."
    ...
    try {
        ...
        npm install
        if ($LASTEXITCODE -ne 0) { Enviar-WhatsApp "npm install falhou."; return }

        npm run build
        if ($LASTEXITCODE -ne 0) { Enviar-WhatsApp "Build falhou."; return }

        vercel --prod --confirm
        if ($LASTEXITCODE -ne 0) { Enviar-WhatsApp "Vercel falhou."; return }

        Escrever-Log "Deploy concluído." "DEPLOY"
        Enviar-WhatsApp "Deploy concluído com sucesso."
    } catch {
        Escrever-Log "Erro no deploy: $_" "ERROR"
        Enviar-WhatsApp "Erro no deploy."
    }
}
...
function Ciclo-Monitor {
    ...
    if ($status -eq "OFFLINE") {
        Escrever-Log "Site OFFLINE → tentar recuperação." "WARN"
        Enviar-WhatsApp "PecuariaTech OFFLINE."
        Fazer-Deploy
    }
}
...
# LOOP 24/7
# ====================================
Escrever-Log "UniScript v24.2.3 iniciado." "INFO"
