<#
🌾 Triângulo360 AutoFix v7.6.1 — Serviço + Dashboard + Instalador
...
$CSV = Join-Path $LOGS "autofix_v761.csv"
$HTML = Join-Path $LOGS "autofix_v761.html"
...
<title>Triângulo360 AutoFix v7.6.1</title>
...
<body>
<h2>🌾 Triângulo360 AutoFix v7.6.1 — Realtime Dashboard</h2>
<div id='pyramid' class='card'></div>
<div id='latency' class='card'></div>
<script>
async function refresh(){
 const res = await fetch('autofix_v761.csv?'+Date.now());
...
if ($Service) {
  ...
  if (!(Get-Service $svc -ErrorAction SilentlyContinue)) {
    $cmd = "pwsh -NoProfile -ExecutionPolicy Bypass -File `"$BASE\Triangulo360-AutoFix-v761.ps1`" -Loop -IntervaloMin 10"
