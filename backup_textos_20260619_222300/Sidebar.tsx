...
import { pecuariaTheme } from "@/app/lib/theme";

const items = [
  { labelPT: "Dashboard", labelES: "Panel", href: "", icon: "🏠" },
  { labelPT: "Financeiro", labelES: "Finanzas", href: "/financeiro", icon: "💰" },
  { labelPT: "Central PecuariaTech – Rebanho", labelES: "Ganado", href: "/Central PecuariaTech – Rebanho", icon: "🐄" },
  { labelPT: "Central PecuariaTech – Pastagem", labelES: "Pastura", href: "/Central PecuariaTech – Pastagem", icon: "🌱" },
  { labelPT: "CFO Inteligente", labelES: "CFO Inteligente", href: "/cfo", icon: "🧠" },
  { labelPT: "Central PecuariaTech – Engorda", labelES: "Engorde", href: "/Central PecuariaTech – Engorda", icon: "⚡" },
  { labelPT: "NURA", labelES: "NURA", href: "/nura", icon: "🧠" },
...
export default function Sidebar({ lang = "pt" }: { lang?: "pt" | "es" }) {
  ...
  return (
    <aside className={`hidden lg:flex w-[250px] flex-col border-r border-[#355845] ${pecuariaTheme.sidebar.background} ${pecuariaTheme.sidebar.text}`}>
      ...
      <div className="p-4">
        <div className="rounded-2xl border border-[#355845] bg-[#173126] p-4 shadow-xl backdrop-blur-sm">
          ...
          <p className="mt-3 text-sm leading-relaxed text-[#b7d6c2]">Plataforma analítica estabilizada</p>
        </div>
      </div>
      <nav className="flex-1 px-4 py-6">
        <div className="space-y-2">
          {items.map((item) => {
            ...
            return (
              <Link key={item.href} href={fullHref} prefetch={false}
                ...
                <span className="font-medium">{lang === "es" ? item.labelES : item.labelPT}</span>
              </Link>
            );
          })}
        </div>
      ...
      <div className="border-t border-[#355845] p-4">
        <div className="rounded-2xl border border-[#355845] bg-[#10251b] p-4">
          <div className="text-xs uppercase tracking-[0.2em] text-[#b7d6c2]">Runtime Cognitivo</div>
          <div className="mt-2 text-sm font-semibold text-[#f3fff7]">Triângulo 360 Online</div>
        </div>
      </div>
    </aside>
  );
