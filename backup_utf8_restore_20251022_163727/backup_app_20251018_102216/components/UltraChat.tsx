...
export default function UltraChat(){
  ...
  async function sendMessage(){
    if (!text.trim()) return;
    const user = { role: 'user', text: text.trim(), ts: new Date().toISOString() };
    ...
    try {
      ...
      if (r.ok && j.reply) {
        ...
        if (j.actionResult) setMessages(m => [...m, { role:'system', text: 'A̤̣o: ' + JSON.stringify(j.actionResult), ts: new Date().toISOString() }]);
      } else {
        setMessages(m => [...m, { role:'assistant', text: 'Erro: ' + (j.error || 'sem resposta'), ts: new Date().toISOString() }]);
      }
    } catch(e) {
      setMessages(m => [...m, { role:'assistant', text: 'Erro de conex̣o', ts: new Date().toISOString() }]);
