<# 
 Deploy-GPS-Supabase-Full_v2.5.ps1
 --------------------------------------------------------
 ✔ Teste de conexão HTTPS real (sem falso negativo)
 ✔ Alerta sonoro e popup vermelho apenas em erro
 ✔ Popup verde de sucesso na verificação inicial
 ✔ Upload automático para Supabase
 ✔ Log CSV consolidado + backup automático
...
function Test-SupabaseReachable {
    try {
        ...
        $r = Invoke-WebRequest -Uri $uri -Method Get -TimeoutSec 10 -UseBasicParsing -ErrorAction Stop
        return $true
    } catch { return $false }
}
...
function Run-MainLoop {
    Write-Host "== Deploy-GPS-Supabase-Full v2.5 ==" -ForegroundColor Cyan
    Ensure-Directories
    Init-CsvLog

    if (Test-SupabaseReachable) {
        Show-Alert -Title "Conexão Supabase" -Message "Conexão HTTPS ativa e verificada com sucesso!" 
    } else {
