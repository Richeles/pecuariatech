<# 
 Deploy-GPS-Supabase-Full_v2.9_Triangulo360.ps1
 --------------------------------------------------------
 🔺 Teste “Triângulo 360°”: DNS + HTTPS + REST
 ✔ Detecta qualquer retorno válido (200/204/401/403)
 ✔ TLS 1.2/1.3 forçado automaticamente
 ✔ Diagnóstico automático de upload
 ✔ Popup verde de sucesso garantido
 ✔ Log e loop contínuo mantidos
#>
...
function Run-MainLoop {
    Write-Host "== Deploy-GPS-Supabase-Full v2.9 (Triângulo 360°) ==" -ForegroundColor Cyan
    Ensure-Directories
    Init-CsvLog
    ...
    while ($true) {
        try {
            $items = Get-Content -Path $QueuePath -ErrorAction SilentlyContinue | Where-Object { $_ -match '\{' }
            foreach ($ln in $items) {
                $ts = Get-Date
