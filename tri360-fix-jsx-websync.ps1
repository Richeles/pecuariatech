Write-Host "🔺 Sistema Triangular 360° — JSX Secure + Web Sync iniciado..." -ForegroundColor Cyan
...
# === Função de correção JSX ===
function Corrigir-JSX {
    param([string]$arquivo)
    $conteudo = Get-Content $arquivo -Raw
    $original = $conteudo

    # Corrige sintaxe incorreta em JSX
    $conteudo = $conteudo -replace "\$\{(\w+)\}", "`${$1}"
    $conteudo = $conteudo -replace "Ãƒ", "Á"
    $conteudo = $conteudo -replace "Ã¢â‚¬Å“", '"'
    $conteudo = $conteudo -replace "Ã¢â‚¬Â", '"'
    $conteudo = $conteudo -replace "Ã¢â‚¬â€", "-"
...
'@ | Out-File $smart -Encoding utf8
    Log "🧩 SmartWeather.tsx reescrito."
}
...
'@ | Out-File $kpi -Encoding utf8
    Log "🧩 Kpi.tsx reescrito."
}

# === Etapa 3: Build ===
Log "🧱 [3/5] Testando build..."
npm run build | Tee-Object -FilePath $logFile -Append

# === Etapa 4: Envio de logs ao Supabase ===
...
$logData = @{
    ...
    mensagem = "Tri360 JSX WebSync executado com sucesso"
    sucesso = $true
...
try {
    ...
    Log "✅ Log enviado ao Supabase com sucesso."
} catch {
    Log "⚠️ Falha ao enviar log: $($_.Exception.Message)"
}

# === Etapa 5: Web Sync — Verifica domínio ===
Log "🌐 [5/5] Verificando domínio online..."
try {
    $resposta = Invoke-WebRequest -Uri $dominio -UseBasicParsing -TimeoutSec 15
    if ($resposta.StatusCode -eq 200) {
        Log "✅ Domínio $dominio acessível e sincronizado com Vercel."
    } else {
        Log "⚠️ Domínio respondeu com código HTTP: $($resposta.StatusCode)"
    }
} catch {
    Log "❌ Falha ao acessar o domínio: $($_.Exception.Message)"
}

Log "📁 Log final salvo em: $logFile"
Write-Host "✅ Tri360 JSX WebSync concluído com sucesso!" -ForegroundColor Green
